...
func Test_WhenTestConfig_ThenConfigIsLoaded(t *testing.T) {
	...
	assert.Equal(t, "9081", cfg.App.Port)
	assert.Equal(t, "info", cfg.App.LogLevel)
	assert.Equal(t, "localhost", cfg.Storage.Database.Host)
	assert.Equal(t, 5432, cfg.Storage.Database.Port)
	assert.Equal(t, "aibuddy_hub_test", cfg.Storage.Database.Name)
	assert.Equal(t, "test", cfg.Storage.Database.User)
	assert.Equal(t, "test", cfg.Storage.Database.Password)
