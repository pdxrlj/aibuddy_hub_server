...
type GenerateAIAgentCallRequest struct {
	...
	InstanceType InstanceType `json:"instance_type,omitempty"` // 互动实例类型
	Config       *AIAgentConfig `json:"config,omitempty"`       // 大模型互动实例配置
}
...
type GenerateAIAgentCallResponse struct {
	...
	InstanceType      string          `json:"instance_type"`        // 大模型互动实例类型
	Context           *AIAgentContext `json:"context,omitempty"`    // 大模型互动实例上下文
}
...
type InterruptRequest struct {
	AppID             string // 大模型互动应用ID（可选，默认使用配置）
	AiAgentInstanceID uint64 `json:"ai_agent_instance_id"` // 大模型互动实例ID
	ExtraMsg          string `json:"extra_msg,omitempty"`  // 打断时携带的播报消息
}
