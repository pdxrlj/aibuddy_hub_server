...
func TestFileProxy(t *testing.T) {
	...
	t.Run("完整下载", func(t *testing.T) {
		...
		file, err := fileHandler.Service.FileProxy(ctx, "", filename, "", "")
	...
	t.Run("分片下载-前1024字节", func(t *testing.T) {
		...
		file, err := fileHandler.Service.FileProxy(ctx, "", filename, "bytes=0-1023", "")
		assert.NoError(t, err)
		assert.NotNil(t, file)
		defer func() { _ = file.Close() }()
	...
	t.Run("分片下载-从1024开始到末尾", func(t *testing.T) {
		...
		file, err := fileHandler.Service.FileProxy(ctx, "", filename, "bytes=1024-", "")
		assert.NoError(t, err)
		assert.NotNil(t, file)
		defer func() { _ = file.Close() }()
	...
	t.Run("分片下载-最后500字节", func(t *testing.T) {
		...
		file, err := fileHandler.Service.FileProxy(ctx, "", filename, "bytes=-500", "")
		assert.NoError(t, err)
		assert.NotNil(t, file)
		defer func() { _ = file.Close() }()
	...
	t.Run("分片下载-中间范围", func(t *testing.T) {
		...
		file, err := fileHandler.Service.FileProxy(ctx, "", filename, "bytes=1000-2000", "")
		assert.NoError(t, err)
		assert.NotNil(t, file)
		defer func() { _ = file.Close() }()
