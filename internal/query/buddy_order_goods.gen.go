...
func newOrderGoods(db *gorm.DB, opts ...gen.DOOption) orderGoods {
	...
	_orderGoods.GoodsPrice = field.NewInt64(tableName, "goods_price")
	_orderGoods.GoodsNum = field.NewInt64(tableName, "goods_num")
	_orderGoods.CreatedAt = field.NewField(tableName, "created_at")
	_orderGoods.UpdatedAt = field.NewField(tableName, "updated_at")
	_orderGoods.GoodsInfo = orderGoodsBelongsToGoodsInfo{
...
type orderGoods struct {
	...
	GoodsInfo  orderGoodsBelongsToGoodsInfo
...
func (o *orderGoods) updateTableName(table string) *orderGoods {
	...
	o.GoodsPrice = field.NewInt64(table, "goods_price")
...
func (o orderGoods) clone(db *gorm.DB) orderGoods {
	...
	o.GoodsInfo.db = db.Session(&gorm.Session{Initialized: true})
	o.GoodsInfo.db.Statement.ConnPool = db.Statement.ConnPool
	return o
}

func (o orderGoods) replaceDB(db *gorm.DB) orderGoods {
	o.orderGoodsDo.ReplaceDB(db)
	o.GoodsInfo.db = db.Session(&gorm.Session{})
	return o
}

type orderGoodsBelongsToGoodsInfo struct {
	db *gorm.DB

	field.RelationField
}

func (a orderGoodsBelongsToGoodsInfo) Where(conds ...field.Expr) *orderGoodsBelongsToGoodsInfo {
	if len(conds) == 0 {
		return &a
	}
...
func (a orderGoodsBelongsToGoodsInfo) WithContext(ctx context.Context) *orderGoodsBelongsToGoodsInfo {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a orderGoodsBelongsToGoodsInfo) Session(session *gorm.Session) *orderGoodsBelongsToGoodsInfo {
	a.db = a.db.Session(session)
	return &a
}

func (a orderGoodsBelongsToGoodsInfo) Model(m *model.OrderGoods) *orderGoodsBelongsToGoodsInfoTx {
	return &orderGoodsBelongsToGoodsInfoTx{a.db.Model(m).Association(a.Name())}
}

func (a orderGoodsBelongsToGoodsInfo) Unscoped() *orderGoodsBelongsToGoodsInfo {
	a.db = a.db.Unscoped()
	return &a
}

type orderGoodsBelongsToGoodsInfoTx struct{ tx *gorm.Association }

func (a orderGoodsBelongsToGoodsInfoTx) Find() (result *model.Goods, err error) {
	return result, a.tx.Find(&result)
}

func (a orderGoodsBelongsToGoodsInfoTx) Append(values ...*model.Goods) (err error) {
...
func (a orderGoodsBelongsToGoodsInfoTx) Replace(values ...*model.Goods) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a orderGoodsBelongsToGoodsInfoTx) Delete(values ...*model.Goods) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a orderGoodsBelongsToGoodsInfoTx) Clear() error {
	return a.tx.Clear()
}

func (a orderGoodsBelongsToGoodsInfoTx) Count() int64 {
	return a.tx.Count()
}

func (a orderGoodsBelongsToGoodsInfoTx) Unscoped() *orderGoodsBelongsToGoodsInfoTx {
	a.tx = a.tx.Unscoped()
	return &a
}
